...
> **Lección aprendida (2026-06-23):** un script de carga con muchas filas (`cargar_puestos.py`, 154 filas en una sola sesión SQLAlchemy reutilizada) se quedó colgado a media corrida contra el proxy público de Railway — sin error, sin avanzar. Diagnóstico: `pg_stat_activity` no mostraba ningún lock real (solo conexiones idle), así que no era un problema de la base de datos; conexiones cortas (conectar → 1 query → cerrar) siempre funcionaron rápido. Conclusión: el proxy público de Railway cierra conexiones inactivas sin avisar al cliente, y `app/database.py` no tenía configurado ningún keepalive/timeout más allá del `connect_timeout` inicial — el cliente se queda esperando una respuesta que nunca llega. **Fix aplicado:** se agregaron `keepalives`/`keepalives_idle`/`keepalives_interval`/`keepalives_count` a `connect_args` en `app/database.py`. Si un script de carga/migración se cuelga de nuevo contra Railway sin error visible, la salida práctica es: cortar el proceso (es seguro si el script comitea fila por fila — es idempotente) y reintentar, o cargar los datos faltantes con conexiones cortas una por fila.
