...
| SAST-05 | JWT en `localStorage`, no revocable | Medium | ❌ Pendiente | sigue en `frontend/src/store/authStore.js` y `frontend/src/lib/axiosClient.js` |
...
- **Bloqueantes de liberación (6):** 5/6 resueltos (contraseña en stdout, autorización por objeto, propiedades destino/continuidad, enumeración bcrypt, throttling). Pendiente: `xlsx`/`python-jose` a versión completa.
- **Siguiente sprint (6):** 0/6 resueltos.
- **Hardening (4):** 0/4 resueltos.

**Total: 5 de 20 hallazgos formales corregidos (SAST-01, SAST-02, SAST-03, SAST-04, SAST-07), 2 parcialmente mitigados (SCA-03/04).** El riesgo dominante que el reporte marcaba como bloqueante —BOLA en lecturas y escrituras de pacientes/médicos (SAST-01/SAST-02)— ya está remediado. Cada hallazgo corregido se verificó con pruebas aisladas de la lógica RBAC contra la base de datos real (sin efectos persistentes) y, cuando aplicó, con llamadas HTTP en vivo.
